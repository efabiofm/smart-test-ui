...
public interface MetodoRepository extends JpaRepository<Metodo,Long> {
    ...
    @Query("select distinct metodo from Metodo metodo left join fetch metodo.parametros")
    List<Metodo> findAllWithEagerRelationships();

    @Query("select metodo from Metodo metodo left join fetch metodo.parametros where metodo.id =:id")
